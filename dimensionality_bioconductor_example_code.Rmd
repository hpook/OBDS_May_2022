---
title: "dimensionality_and_clustering_example_code"
author: "Hannah Pook"
date: '2022-05-18'
...
> Answer:
> EdgeR is more popular (it is higher up in the list, which is ranked by popularity)
> I would consult the vignette if I needed more detail than the help page gives 
...
> Answer:
> This function tells us how many packages are out of date 
...
```
  ...
  - This will open a webpage containing links to package vignettes (although these links don't always work)
  - We can find 5 vignettes 

- Import sequences in the file `TruSeq3-PE-2.fa`.
  What is the class of the object?

  - Here we need to use readDNAStringSet function from the Biostrings package. We can figure this out by looking at the Biostrings package help page. 
...
```
  - This function lists the available genomes 
...
```{r}
Ebox <- "CANNTG"
countPattern(pattern=Ebox, subject=chrY, fixed=FALSE) # fixed false means N in the pattern can match any based
```
...
- How many transcripts do the annotations describe for that gene?
  ...
  - Here we can use unique function to assess unique values in the transcript_id column 
  - setdiff(NA) is used to make sure NA values are not counted as a unique ID
  - Length tells us how many of these unique values there are
...
- Subset the annotations to exons, and use the `split()` function to separate exons arising from different transcripts.
...
> Answer: 
> A different select function (from a different package) may be used

- Use the `mapIds()` method to get the Ensembl identifier for those gene symbols.

  - This is a quick way to get the Ensembl identifiers, but can only look at one column at a time 
  - Returns a vector rather than a table 

```{r}
mapIds(org.Hs.eg.db, keys=human_symbols, column="ENSEMBL", keytype="SYMBOL", multiVals="first")
```
...
```
  ...
  - Using the function listDatasets we can see that the human identifier is hsapiens_gene_ensembl
...
- Use the `getBM()` function to fetch the chromosome, start and end positions, and strand information for the gene symbols that you stored earlier as `human_symbols`.
  ...
  - In attributes we need to list the attributes we would like to obtain for each gene (note: we need to include the gene symbols here for this to be printed as a column)
  - In filters we say what we want to filter by, and in values we specify which genes(symbols) we want to include 
  - With mart we specify the object we want to look at, which we called mart 
```{r}
getBM(
  attributes = c(
    "hgnc_symbol",
...
- This code is to create a matrix used to demonstrate the SummarisedExperiment function
...
colData(demo_se) # can also be used to edit matrix and add data 
rowData(demo_se)
mcols(demo_se)
metadata(demo_se)
```
...
> Answer:
> These functions give us information about the SummarisedExperiment object 
> They also allow us to edit the matrix (e.g. add columns and rows)
...
```{r}
sample_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv", row.names="X")

head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv", row.names="X")
